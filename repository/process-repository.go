...
func (pool ProcessDB) GetOrder(orderId int) (*dto.OrderResponse, error) {
...
func (pool ProcessDB) FinishUpdate(t_, orderId int, finishedDate int64) error {
...
func (pool ProcessDB) Update(t_, orderId int) error {
	q := `Update order SET orderStatus=$1 WHERE id=$2`
...
func (pool ProcessDB) Delete(orderId int) error {
	delete := `Delete From order_process WHERE orderId=$1;`
...
func (pool ProcessDB) AcceptOrder(driverId, orderId int) (interface{}, error) {
	...
	save := `INSERT INTO order_process(
		orderId,
		driverId,
		userid,
		startDate  
...
func (pool ProcessDB) CancellOrder(orderId int) (interface{}, error) {
	order, err := pool.GetOrder(orderId)
...
func (pool ProcessDB) GetOrdersInProcessDriver(driverId int) ([]*models.ProcessModel, error) {
...
func (pool ProcessDB) GetOrdersInProcessUser(userId int) ([]*models.ProcessModel, error) {
...
func (pool ProcessDB) FinishOrder(driverId, orderId int) (interface{}, error) {
	order, err := pool.GetOrder(orderId)
