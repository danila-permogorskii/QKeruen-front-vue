...
import { getDriverHistory, deleteDriverOffer } from '@/api/driver';

export default {
  name: 'DriverOffersHistory',
  components: {
    VCircleButton,
    VButton,
    TripItem,
  ...
  methods: {
    async getDriverHistory() {
      this.isLoading = true;
      try {
        const { data } = await getDriverHistory(this.driverInfo.id);
    ...
    async realExitOffer() {
      ...
      try {
        const response = await deleteDriverOffer(this.exitingOfferId);
